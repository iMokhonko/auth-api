locals {
  ...
  hasEnv           = var.env != "prod"
  dns_record       = "${var.feature == "master" ? "" : "${var.feature}."}api.auth${local.hasEnv ? ".${var.env}" : ""}"
}
...
resource "aws_cloudfront_distribution" "distribution" {
  ...
  default_cache_behavior {
    ...
    origin_request_policy_id   = "b689b0a8-53d0-40ab-baf2-68738e2966ac" // Managed Policy
...
resource "aws_cloudfront_function" "cors_headers" {
  name    = "${var.env}-auth-api-subdomain-cors"
